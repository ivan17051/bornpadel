...
@php
    ...
    $kategoriQuery = ($turnamen && ($hasMultipleKategori ?? false) && ! empty($kategoriId))

        ? ['id_kategori' => $kategoriId]

        : [];

    $tabBase = function (string $tab) use ($turnamen, $kategoriQuery) {

        return array_filter(array_merge(

            ['id_turnamen' => optional($turnamen)->id, 'tab' => $tab],
...
@include('admin.partials.turnamen-filter', [
    ...
    'preserveParams' => array_filter(['tab' => $activeTab]),
...
@if ($turnamen)
    ...
    <ul class="nav nav-tabs mb-3" role="tablist">
        ...
        <li class="nav-item" role="presentation">
            ...
            <a class="nav-link {{ $activeTab === 'pemain' ? 'active' : '' }}"
               ...
               href="{{ route('admin.turnamen-operasi.index', array_filter(array_merge($tabBase('pemain'), ['search' => request('search'), 'status' => request('status'), 'sort' => request('sort'), 'dir' => request('dir')]))) }}">
        ...
        <li class="nav-item" role="presentation">
            ...
            <a class="nav-link {{ $activeTab === 'matchmaking' ? 'active' : '' }}"
               ...
               href="{{ route('admin.turnamen-operasi.index', $tabBase('matchmaking')) }}">
        ...
        <li class="nav-item" role="presentation">
            ...
            <a class="nav-link {{ $activeTab === 'klasemen' ? 'active' : '' }}"
               ...
               href="{{ route('admin.turnamen-operasi.index', $tabBase('klasemen')) }}">
        ...
        @if (! $turnamen->isMahjong() && ! $turnamen->isFriendly())
            ...
            <li class="nav-item" role="presentation">
                ...
                <a class="nav-link {{ $activeTab === 'bracket' ? 'active' : '' }}"
                   ...
                   href="{{ route('admin.turnamen-operasi.index', $tabBase('bracket')) }}">
    ...
    <div class="tab-content" @if (! empty($kategoriId)) data-workspace-kategori="{{ $kategoriId }}" @endif>
        ...
        @elseif ($activeTab === 'matchmaking')
            ...
            @include('admin.matchmaking.partials.workspace', [
                ...
                'bracketUrl' => route('admin.turnamen-operasi.index', $tabBase('bracket')),
